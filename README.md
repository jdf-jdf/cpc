...
A 23-slide beginners' AI training, built to be delivered live to a small,
internal, English-speaking CPC team. Browser-based, no build step, no
dependencies. Open `index.html` and present.

```
index.html            all 23 slides and their presenter notes
assets/
  css/deck.css
  js/deck.js
  img/cpc-logo.png
...
| <kbd>1</kbd>–<kbd>23</kbd> | Jump to a slide by number |
...
- **Slide 21 — "What would have to be true before you would trust this with
...
- **There is no written AI data rule at CPC to quote, so slide 16 is the rule.**
  ...
  slide 21. Do not imply it has been approved somewhere it has not.
- **Using Claude today is not CPC approving Claude.** Slide 21 still asks which
...
sorted, run slide 20 on the screen with the room calling out the prompt; it
...
23 slides, roughly 60–90 minutes depending on how long the discussions run.
...
**18–19.** Describing instead of pasting, so the red rule has a workable
alternative. Then asking well, in four parts — which is slide 5's role-playing
point made practical: you are not typing a question, you are setting up who is
answering.

**20.** Hands-on, ten minutes on a real CPC task: summarising an RFP, tightening
a grant paragraph, budget justification text, a run-of-show.

**21–23.** What CPC still has to decide, five house rules, questions.
...
slide 21 as an open question rather than stated as settled.
